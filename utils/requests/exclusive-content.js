export const postExclusiveContentReq = async (_token, _reqData) => {
  const response = await fetch(
    `https://aks.lynq.app/legacy/exclusive-content/adm?t=${_token}`,
    {
      method: 'POST',
      headers: {
...
export const postExclusiveContentFileReq = async (_id, _token, _file) => {
  ...
  const response = await fetch(
    `https://aks.lynq.app/legacy/exclusive-content/adm/upload/${_id}?t=${_token}`,
    {
      method: 'POST',
      body: formData,
    },
...
export const putExclusiveContentReq = async (_token, _id, _reqData) => {
  const response = await fetch(
    `https://aks.lynq.app/legacy/exclusive-content/adm/${_id}?t=${_token}`,
    {
      method: 'PUT',
      headers: {
        Accept: 'application/json',
